...
=======
>>>>>>> 6e8f101d1a5b6c2c722f1feeaf27bbd6f0d4c047
...
const exportToExcel = async () => {
    try {
        ...
        const response = await api.get('/user/cirugias/exportar-todos/', {
            params: {
        ...
        const excelData = data.map(item => ({
            ...
            'N° HCL': item.n_hcl,
            'Fecha IQX Programada': formatDateForExcel(item.fecha_iqx_programada),
            'Código IQX Programada': item.codigo_iqx_programada,
            'IQX Programada': item.iqx_programada,
            'Fecha IQX Realizada': formatDateForExcel(item.fecha_iqx_realizada),
            'Código IQX Realizada': item.codigo_iqx_realizada,
            'IQX Realizada': item.iqx_realizada,
            '¿Se Reprogramó?': item.se_reprogramo,
            'Fecha IQX Reprogramada': formatDateForExcel(item.fecha_iqx_reprogramada),
            'Motivo Reprogramación': formatDateForExcel(item.motivo_reprogramacion), // <-- si es texto, cámbialo a item.motivo_reprogramacion directamente
            'Fecha Realizada IQX Reprogramada': formatDateForExcel(item.fecha_realizada_iqx_reprogramada),
            'Código IQX Reprogramada': item.codigo_iqx_reprogramada,
            'IQX Reprogramada': item.iqx_reprogramada,
            'Creado Por': item.creado_por?.username || 'N/A',
            'Fecha Creación': formatDateForExcel(item.fecha_creacion),
            'Fecha Actualización': formatDateForExcel(item.fecha_actualizacion),
        }));
...
const uploadFile = async () => {
    ...
    try {
        ...
        const response = await api.post('/user/cirugias/importar-excel/', formData, {
            headers: {
...
const cargarRegistros = async () => {
    try {
        ...
        const response = await api.get('/user/cirugias/', { params })
